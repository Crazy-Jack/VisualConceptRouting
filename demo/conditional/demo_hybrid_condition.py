...
from utils.data_io import LoadDataSet, LoadDataSet_condition
...
special='hybrid_conditional_layer_con_{}'.format(layer_condition)
dataname='mylsunbed100k64_and_deepfashion'
...
bedroom_pathlist = [(os.path.join(datapath[0], i), 0) for i in os.listdir(datapath[0])]
...
deepfashion_pathlist = [(os.path.join(deepfashion_data_folder, i), 1) for i in list(deepfashion_index_file.iloc[:,0])]
datapathlist = deepfashion_pathlist + bedroom_pathlist


class_num = 2
z_dim = 100
train_set, labels = LoadDataSet_condition(datapathlist, im_size=[IMG_HEIGHT,IMG_WIDTH,CHANNELS], class_num=class_num, mode=2)
...
lbls = tf.placeholder(shape=[BATCH_SIZE, class_num], dtype=tf.float32, name='lbls')
...
def P(z, class_info, reuse=False, layer_condition=False):
    with tf.variable_scope('gen', reuse=reuse):
        ...
        fmd2=block_up_no(fmsparsel1,channelnd[1],k=kersized[0], s=1,name='block1', class_info=class_info, layer_condition=layer_condition) #8x8
        fmsparsel2=sparseop(fmd2,int(1/4*fmd[1]*fmd[1]),1)
        fmd3 = block_up_no(fmsparsel2,channelnd[2],k=kersized[1], s=1,name='block2', class_info=class_info, layer_condition=layer_condition)#16x16
        fmsparsel3 = sparseop(fmd3,int(1/4*fmd[2]*fmd[2]),1)
        fmd4 = resblock_up_no(fmsparsel3,channelnd[3],k=kersized[2], s=1,name='block3', class_info=class_info, layer_condition=layer_condition)#32x32
        fmsparsel4=sparseop(fmd4,int(1/3*fmd[3]*fmd[3]),1)
        fmd5=block_up_no(fmsparsel4,channelnd[4],k=kersized[3], s=1,name='block4', class_info=class_info, layer_condition=layer_condition)#64x64
...
with tf.Session(config=config) as sess:
    ...
    for iepoch in range(Nepoch+1):
        rand_index = np.random.permutation(np.arange(len(train_set)))
        train_set_rp=train_set[rand_index]
        ...
        for i in tqdm(range(Nbatch), total=Nbatch):
            ...
            batch_labels = labels_rp[i * BATCH_SIZE:min(len(train_set), (i + 1) * BATCH_SIZE)]
            #Yp=localCN_v2(Ypori)
            Yp=Ypori
            ...
            _,lossdp,GPp,DRegp,energy_realp,energy_synp,lossgp1=sess.run(opdesrun,feed_dict={Y: Yp,z_s:z_sp,lr_d:lr_dp, lbls: batch_labels})
