...
def set_args():
    ...
    parser.add_argument('--vae_weight', type=float, default=10, help='weight for vae')
...
def train_encoder_generator(train_loader, models, optim_encoder, optim_generator, l2_reconstruct_criterion, args):
    ...
    for batch_id, img in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        z_a, mu_a, logvar_a = models.encoder(img)
        vae_loss = vae_loss_function(mu_a, logvar_a)
        ...
        # final loss
        loss = reconstruct_loss * args.recon_weight + critic_loss + vae_loss * args.vae_weight
        # update
        loss.backward()
